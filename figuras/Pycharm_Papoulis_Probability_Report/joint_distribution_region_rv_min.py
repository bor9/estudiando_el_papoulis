...
# labels
plt.text(t_min/2, z/2, r'$y\leq w$', fontsize=font_size, ha='center', va='center')
plt.text(t_min, t_max/2, r'$y=w$', fontsize=font_size, ha='center', va='center')

plt.text(t_max-3, t_max-4, r'$x=y$', fontsize=font_size, ha='center', va='bottom', rotation=45)
plt.text((t_max+z)/2+1, z+1, r'$x>y$', fontsize=font_size, ha='center', va='bottom')

plt.text(z, z+0.4, r'$(w,\,w)$', fontsize=font_size, ha='right', va='bottom')
plt.plot(z, z, 'k.', markersize=8)

plt.text((t_min+t_max)/2, ax_min-3.5, r'$P\{\mathbf{y}\leq w,\,\mathbf{x}>\mathbf{y}\}$',
...
# labels
plt.text((t_max+z)/2-1, t_max-1, r'$x\leq y$', fontsize=font_size, ha='center', va='top')
plt.text(z, t_max, r'$x=w$', fontsize=font_size, ha='center', va='bottom')

plt.text(t_max, t_max-4, r'$x=y$', fontsize=font_size, ha='center', va='bottom', rotation=45)
plt.text(z/2, t_min/2, r'$x\leq w$', fontsize=font_size, ha='center', va='center')

plt.text(z+0.4, z-0.7, r'$(w,\,w)$', fontsize=font_size, ha='left', va='center')
plt.plot(z, z, 'k.', markersize=8)

plt.text((t_min+t_max)/2, ax_min-3.5, r'$P\{\mathbf{x}\leq w,\,\mathbf{x}\leq \mathbf{y}\}$',
         fontsize=font_size, ha='center', va='baseline')
...
plt.plot([z, z], [z, t_max], 'k')
plt.plot([z, t_max], [z, z], 'k')
# filled region (x<y)
vertices = np.array([[z, z], [z, t_max], [t_min, t_max], [t_min, t_min], [t_max, t_min], [t_max, z]])
ax.add_patch(Polygon(vertices, facecolor='#0343df', alpha=0.4, edgecolor='none'))
vertices = np.array([[z, z], [z, t_max], [t_min, t_max], [t_min, t_min], [t_max, t_min], [t_max, z]])
ax.add_patch(Polygon(vertices, facecolor='#ff000d', alpha=0.4, edgecolor='none'))

# labels
plt.text(z+0.4, z+0.4, r'$(w,\,w)$', fontsize=font_size, ha='left', va='bottom')
plt.plot(z, z, 'k.', markersize=8)

plt.axis('off')

plt.text((t_min+t_max)/2, ax_min-3.5, r'$P\{(\mathbf{y}\leq w,\,\mathbf{x}> \mathbf{y})'
                                      r'\cup(\mathbf{x}\leq w,\,\mathbf{x}\leq \mathbf{y})\}$',
         fontsize=font_size, ha='center', va='baseline')
...
plt.legend(handles=handles, labels=["Región 1", "Región 2", "Intersección"], framealpha=1, frameon=False,
           loc='center left', bbox_to_anchor=(0.82, 0.85))

plt.savefig('joint_distribution_region_rv_min.pdf', bbox_inches='tight')
